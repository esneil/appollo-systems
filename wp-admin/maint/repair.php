...
if ( ! defined( 'WP_ALLOW_REPAIR' ) ) {
	...
	if ( $duplicated_keys || $missing_key ) {
		...
		// Translators: 1: wp-config-back.php; 2: Secret key service URL.
		echo '<p>' . sprintf( __( 'While you are editing your %1$s file, take a moment to make sure you have all 8 keys and that they are unique. You can generate these using the <a href="%2$s">WordPress.org secret key service</a>.' ), '<code>wp-config-back.php</code>', 'https://api.wordpress.org/secret-key/1.1/salt/' ) . '</p>';
	}
	...
	if ( $problems ) {
		...
		echo '<p>' . __( 'Repairs complete. Please remove the following line from wp-config-back.php to prevent this page from being used by unauthorized users.' ) . "</p><p><code>define('WP_ALLOW_REPAIR', true);</code></p>";
	}
} else {
